    ...
    <div className="space-y-8 py-4">
      <section className="rounded-3xl border border-border-light/70 bg-surface-light p-6 shadow-sm dark:border-border-dark/70 dark:bg-surface-dark">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="rounded-2xl border border-border-light/60 bg-background-light/70 px-4 py-2 text-sm text-text-secondary-light dark:border-border-dark/60 dark:bg-background-dark/40 dark:text-text-secondary-dark">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        {departmentOverview.strengths.map((item) => (
          <div
            key={item}
            className="rounded-2xl border border-border-light/70 bg-surface-light p-4 text-sm shadow-sm dark:border-border-dark/70 dark:bg-surface-dark"
          >
      ...
      <section className="grid gap-6 lg:grid-cols-[2fr,1fr]">
        <div className="space-y-3 rounded-3xl border border-border-light/70 bg-surface-light p-6 shadow-sm dark:border-border-dark/70 dark:bg-surface-dark">
          ...
          <ul className="space-y-3 text-sm text-text-secondary-light dark:text-text-secondary-dark">
            {personalizedTips.general.map((tip) => (
              <li
                key={tip}
                className="rounded-2xl border border-border-light/60 bg-background-light/70 p-4 dark:border-border-dark/60 dark:bg-background-dark/40"
              >
                {tip}
              </li>
            ))}
        ...
        <div className="space-y-3 rounded-3xl border border-border-light/70 bg-surface-light p-6 text-sm shadow-sm dark:border-border-dark/70 dark:bg-surface-dark">
          <h3 className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">
            졸업 체크리스트
          </h3>
          <ul className="list-disc space-y-2 pl-5 text-text-secondary-light dark:text-text-secondary-dark">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {syllabusHighlights.map((category) => (
            ...
            >
              ...
              <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-text-secondary-light dark:text-text-secondary-dark">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-3 rounded-3xl border border-border-light/70 bg-surface-light p-6 shadow-sm dark:border-border-dark/70 dark:bg-surface-dark">
          ...
          <ul className="list-disc space-y-2 pl-5 text-sm text-text-secondary-light dark:text-text-secondary-dark">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {departmentResources.map((resource) => (
            <div
              key={resource.title}
              className="flex flex-col rounded-2xl border border-border-light/70 bg-surface-light p-4 shadow-sm dark:border-border-dark/70 dark:bg-surface-dark"
            >
              <p className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">
                {resource.title}
              </p>
