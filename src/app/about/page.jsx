...
const contributors = [
  ...
  {
    name: "정찬호",
    role: "게시판·자료실 인터랙션 · 카테고리/조회수 표시, 파일 미리보기 흐름 정돈",
  },
  {
    name: "이은우",
...
export default function AboutPage() {
  return (
    <div className="space-y-10 py-6">
      <section className="rounded-3xl border border-border-light/70 bg-surface-light p-8 shadow-sm">
        <p className="text-sm font-semibold text-primary">컴파일룸 안내</p>
        <h1 className="mt-2 text-3xl font-bold text-text-primary-light">
          전주대학교 컴퓨터공학과 통합 플랫폼
        </h1>
        <p className="mt-3 text-sm leading-relaxed text-text-secondary-light">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-3xl border border-border-light/70 bg-background-light/80 p-6">
          <h2 className="text-xl font-semibold text-text-primary-light">
            컴파일룸에서 할 수 있는 것
          </h2>
          <ul className="mt-3 space-y-2 text-sm text-text-secondary-light">
            {platformHighlights.map((item) => (
              <li key={item} className="rounded-2xl bg-surface-light/70 px-4 py-2">
                {item}
              </li>
            ))}
          </ul>
      ...
      <section className="rounded-3xl border border-border-light/70 bg-surface-light p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {contributors.map((member) => (
            <div
              key={member.name}
              className="rounded-2xl border border-border-light/70 bg-background-light/60 px-4 py-3 text-sm"
            >
      ...
      <section className="rounded-3xl border border-dashed border-border-light/70 bg-background-light/70 p-6 text-sm text-text-secondary-light">
        ...
        <div className="mt-4 flex gap-3">
          <Link
            href="/"
            className="rounded-full border border-border-light/70 px-4 py-2 text-sm font-semibold text-text-primary-light transition hover:border-primary/40 hover:text-primary"
          >
            홈으로 돌아가기
          </Link>
          <a
